...
func filterMessages(messages []Message, filterType string) []Message {
	returnedSlice := []Message{}
	for _, message := range messages {
		if message.Type() == filterType {
			returnedSlice = append(returnedSlice, message)
		}
	}
...
For this feature, messages are categorized into three types: TextMessage, MediaMessage, and LinkMessage.
