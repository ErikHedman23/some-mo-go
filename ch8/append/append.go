...
func getCostsByDay(costs []cost) []float64 {
	...
	// hint: use append
	// hint: use the length of the highest day
	for i := 0; i < len(costs); i++ {
		if costs[i].day > highestDay {
			highestDay = costs[i].day
		}
	...
	for day := 0; day < highestDay+1; day++ {
		dayCost := 0.0

		for i := 0; i < len(costs); i++ {
			if costs[i].day == day {
				dayCost += costs[i].value
			}
		}
		output = append(output, dayCost)
	}
